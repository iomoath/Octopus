...
class NewListener:
    ...
    def start_listener(self):
        ...
        if self.arguments == 8:
            ...
            thread.daemon = True
            thread.start()
        listeners_information[self.name] = [self.name, self.bindip, self.bindport, self.host, self.interval, self.path,
                                            self.ssl]
...
'''
        ...
        js = js.replace('{ip}', self.host).replace('{port}', str(self.bindport)).replace('{payload}',
                                                                                         self.path).replace(
            "{protocol}", protocol)
        # print  js
